...
func DeployIBC(
	...
	if err := generateGenesis("groth16", "node_modules/@cosmos/solidity-ibc-eureka/scripts/genesis.json"); err != nil {
		return DeployedContracts{}, err
	}

	stdout, err := runDeployScript(ethRPC, deployerPrivKey, faucetPubKey)
	if err != nil {
		return DeployedContracts{}, fmt.Errorf("failed deploy IBC: %w", err)
	}
...
func runDeployScript(
	...
	cmd := exec.Command(
		"forge",
		"script",
		"node_modules/@cosmos/solidity-ibc-eureka/scripts/E2ETestDeploy.s.sol:E2ETestDeploy",
		"--rpc-url", ethRPC,
	...
	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("failed to run forge script: %w", err)
	}
...
func getEthContractsFromDeployOutput(stdout string) (DeployedContracts, error) {
	...
	if err != nil {
		return DeployedContracts{}, fmt.Errorf("failed to unmarshal JSON for ETH contract extraction: %w", err)
	}
