...
func (i ImageRef) PullImage(ctx context.Context, client *client.Client) error {
	ref := i.Ref()
...
func NewImage(logger *zap.Logger, cli *client.Client, networkID string, cleanupLabel string, imageRef ImageRef) *Image {
	if logger == nil {
	...
	i.log = logger.With(
		...
		zap.String("test_name", cleanupLabel),
	)
	return i
}
...
func (image *Image) CreateContainer(ctx context.Context, containerName, hostName string, cmd []string, opts ContainerOptions) (string, error) {
	...
	cc, err := image.client.ContainerCreate(
		...
		&container.Config{
			...
			Labels: map[string]string{CleanupLabel: image.CleanupLabel},
		},
...
func (image *Image) Start(ctx context.Context, cmd []string, opts ContainerOptions) (*Container, error) {
	...
	var (
		containerName = SanitizeContainerName(image.CleanupLabel + "-" + RandLowerCaseLetterString(6))
