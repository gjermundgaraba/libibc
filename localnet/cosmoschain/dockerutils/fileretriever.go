...
func NewFileRetriever(log *zap.Logger, cli *client.Client, cleanupLabel string) *FileRetriever {
	return &FileRetriever{log: log, cli: cli, cleanupLabel: cleanupLabel}
}
...
func (r *FileRetriever) SingleFileContent(ctx context.Context, volumeName, relPath string) ([]byte, error) {
	...
	cc, err := r.cli.ContainerCreate(
		...
		&container.Config{
			...
			Labels: map[string]string{CleanupLabel: r.cleanupLabel},
		},
