...
const (
	...
	SP1ProverTypeNetwork SP1ProverType = "network"
)
...
func NewConfig(logLevel string, port int, modules []ModuleConfig) Config {
	return Config{
		LogLevel: logLevel,
		Address:  "127.0.0.1",
		Port:     port,
...
func (c *Config) GenerateConfigFile(filePath string) error {
	tmpl, err := template.ParseFiles("eurekarelayer/config.tmpl")
	if err != nil {
		return err
	}
