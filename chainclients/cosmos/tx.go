package cosmos

import (
	"context"
	"time"

	"github.com/cosmos/cosmos-sdk/client/tx"
	sdk "github.com/cosmos/cosmos-sdk/types"
	txtypes "github.com/cosmos/cosmos-sdk/types/tx"
	"github.com/cosmos/cosmos-sdk/types/tx/signing"
	xauthsigning "github.com/cosmos/cosmos-sdk/x/auth/signing"
	authtx "github.com/cosmos/cosmos-sdk/x/auth/tx"
	accounttypes "github.com/cosmos/cosmos-sdk/x/auth/types"
	"github.com/cosmos/gogoproto/proto"
	"github.com/gjermundgaraba/libibc/chainclients/network"
	"github.com/gjermundgaraba/libibc/chainclients/utils"
	"github.com/pkg/errors"
	"go.uber.org/zap"
)

var _ network.NewTx = &CosmosNewTx{}

type CosmosNewTx struct {
	bz []byte
}

func NewCosmosNewTx(bz []byte) *CosmosNewTx {
	return &CosmosNewTx{
		bz: bz,
	}
}

func (c *CosmosNewTx) GetTxBytes() []byte {
	return c.bz
}

// SubmitTx implements network.Chain.
func (c *Cosmos) SubmitTx(ctx context.Context, newTx *CosmosNewTx, wallet network.Wallet, gas uint64) (string, error) {
	cosmosWallet, ok := wallet.(*Wallet)
	if !ok {
		return "", errors.Errorf("invalid wallet type: %T", wallet)
	}

	// Extract messages from the response (cosmos specific)
	var txBody txtypes.TxBody
	if err := proto.Unmarshal(newTx.GetTxBytes(), &txBody); err != nil {
		return "", err
	}

	txJson, err := c.codec.MarshalJSON(&txBody)
	if err != nil {
		// this is only used for debugging, so we just log it
		c.logger.Error("failed to marshal tx body to json", zap.Error(err))
	}
	c.logger.Debug("tx body", zap.String("tx_body", string(txJson)))

	if len(txBody.Messages) == 0 {
		return "", errors.New("no messages in tx")
	}

	var msgs []sdk.Msg
	for _, msg := range txBody.Messages {
		var sdkMsg sdk.Msg
		if err := c.codec.InterfaceRegistry().UnpackAny(msg, &sdkMsg); err != nil {
			return "", err
		}

		msgs = append(msgs, sdkMsg)
	}

	txResp, err := c.submitTx(ctx, cosmosWallet, gas, msgs...)
	if err != nil {
		return "", errors.Wrap(err, "failed to submit tx")
	}

	return txResp.TxHash, nil
}

func (c *Cosmos) submitTx(ctx context.Context, wallet *Wallet, gas uint64, msgs ...sdk.Msg) (*sdk.TxResponse, error) {
	grpcConn, err := utils.GetGRPC(c.grpcAddr)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get grpc connection")
	}

	// Get account for sequence and account number
	accountClient := accounttypes.NewQueryClient(grpcConn)
	accountRes, err := accountClient.AccountInfo(ctx, &accounttypes.QueryAccountInfoRequest{Address: wallet.Address()})
	if err != nil {
		return nil, errors.Wrap(err, "failed to get account info")
	}

	exp := time.Date(2025, time.April, 15, 15, 0, 0, 0, time.UTC)
	_ = exp

	txCfg := authtx.NewTxConfig(c.codec, authtx.DefaultSignModes)
	txBuilder := txCfg.NewTxBuilder()
	txBuilder.SetGasLimit(gas)
	txBuilder.SetMsgs(msgs...)
	gasPrice := int64(float64(gas) * c.GasPrices)
	txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewInt64Coin(c.GasDenom, gasPrice)))

	sigV2 := signing.SignatureV2{
		PubKey: wallet.privateKey.PubKey(),
		Data: &signing.SingleSignatureData{
			SignMode:  signing.SignMode(txCfg.SignModeHandler().DefaultMode()),
			Signature: nil,
		},
		Sequence: accountRes.Info.Sequence,
	}
	err = txBuilder.SetSignatures(sigV2)
	if err != nil {
		return nil, errors.Wrap(err, "failed to set signature")
	}

	signerData := xauthsigning.SignerData{
		Address:       wallet.Address(),
		ChainID:       c.ChainID,
		AccountNumber: accountRes.Info.AccountNumber,
	}
	sigV2, err = tx.SignWithPrivKey(
		ctx,
		signing.SignMode(txCfg.SignModeHandler().DefaultMode()),
		signerData,
		txBuilder,
		wallet.privateKey,
		txCfg,
		accountRes.Info.Sequence,
	)
	if err != nil {
		return nil, errors.Wrap(err, "failed to sign with priv key")
	}
	err = txBuilder.SetSignatures(sigV2)
	if err != nil {
		return nil, errors.Wrap(err, "failed to set signature")
	}

	// Generated Protobuf-encoded bytes.
	txBytes, err := txCfg.TxEncoder()(txBuilder.GetTx())
	if err != nil {
		return nil, errors.Wrap(err, "failed to encode tx")
	}

	txClient := txtypes.NewServiceClient(grpcConn)
	// We then call the BroadcastTx method on this client.
	broadcastResp, err := txClient.BroadcastTx(
		ctx,
		&txtypes.BroadcastTxRequest{
			Mode:    txtypes.BroadcastMode_BROADCAST_MODE_SYNC,
			TxBytes: txBytes, // Proto-binary of the signed transaction, see previous step.
		},
	)
	if err != nil {
		return nil, errors.Wrap(err, "failed to broadcast tx")
	}
	if broadcastResp.TxResponse.Code != 0 {
		return nil, errors.Errorf("tx failed with code %d: %+v", broadcastResp.TxResponse.Code, broadcastResp.TxResponse)
	}

	c.logger.Info("tx broadcasted", zap.String("tx_hash", broadcastResp.TxResponse.TxHash))

	// Just to give it some time to index the tx properly
	resp := &sdk.TxResponse{}
	err = utils.WaitForCondition(time.Second*30, time.Second*5, func() (bool, error) {
		txResp, err := txClient.GetTx(ctx, &txtypes.GetTxRequest{Hash: broadcastResp.TxResponse.TxHash})
		if err != nil {
			c.logger.Debug("failed to query while waiting after broadcast tx", zap.String("tx_hash", broadcastResp.TxResponse.TxHash), zap.Error(err))
			return false, nil
		}
		if txResp.TxResponse.Code != 0 {
			return false, errors.Errorf("tx %s failed with code %d: raw_log=%s", broadcastResp.TxResponse.TxHash, txResp.TxResponse.Code, txResp.TxResponse.RawLog)
		}

		if err := c.codec.UnmarshalJSON(txResp.TxResponse.Tx.Value, resp); err != nil {
			return false, errors.Wrap(err, "failed to unmarshal tx response")
		}

		return true, nil
	})

	return resp, nil
}
