...
import (
	...
	"github.com/gjermundgaraba/libibc/chainclients/ibc"
...
func (c *Cosmos) SendTransfer(
	...
	packets, err := c.GetPackets(ctx, resp.TxHash)
	if err != nil {
		return ibc.Packet{}, errors.Wrapf(err, "failed to get packets for transfer with tx hash: %s", resp.TxHash)
	}
	if len(packets) != 1 {
		return ibc.Packet{}, errors.Errorf("failed to get packet for transfer (expected 1, got %d)", len(packets))
	}

	c.logger.Info("Sent transfer", zap.String("tx_hash", resp.TxHash), zap.String("from", wallet.Address()), zap.String("to", to), zap.Uint64("amount", amount.Uint64()), zap.String("denom", denom))
